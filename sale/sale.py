...
class sale_order(osv.osv):
    ...
    def _find_vehicle(self, cr, uid, ids, context=None):
        for vehicle in self.browse(cr, uid, ids, context=context):
            v = vehicle.vehicle_no.vehicle_no
  ...
  - The 'Invoice From The Picking' choice is used to create an invoice during the picking process."""),
        ...
        'vehicle_no': fields.many2one('weighbridge.info', 'Vehicle Num')    
    }
    _defaults = {
...
class sale_order_line(osv.osv):
    ...
    def button_confirm(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            tweight += _get_line_qty(line)

        
        v_tweight = line.order_id.vehicle_no.gross_weight 
        v_weightin = line.order_id.vehicle_no.weight_in  
	v_weightout=line.order_id.vehicle_no.weight_out 
        w_allowed = (v_tweight - v_weightin-v_weightout)*1.1
        v_purpose = line.order_id.vehicle_no.purpose 

        
        if(v_purpose == "s"):
            if(tweight > w_allowed):
                w_overload = tweight - w_allowed
                raise osv.except_osv(('Error'), 'The selected vehicle will not be able to carry this order. Excess weight=' + str(w_overload) + ' Kg')
            else:
		
                return self.write(cr, uid, ids, {'state': 'confirmed'})
        else:
            raise osv.except_osv(('Error'), 'The vehicle has NOT been registered for sale order.')
